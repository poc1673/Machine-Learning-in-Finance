...
["rules",[h,null,[[function(c,b){var d=[],a;if("array"==={}.toString.call(b).match(/\s([a-zA-Z]+)/)[1].toLowerCase()){for(a=0;a<b.length;a++)d.push(c+"."+b[a]);return{labels:d.join(",")}}return{labels:c+"."+b}},"Title"]],[[function(c,b,d){b=document.getElementsByTagName("meta");for(var a,e,f=[],g=0;g<b.length;g++)a=b[g],e=a.getAttribute("name")||a.getAttribute("property"),e==d&&(f=f.concat(k(d,a.getAttribute("content"))));0<f.length?c(f):c(!1)},"exactmatch","og:title"]]]])})("p-3bmCtEkzhulVU",window,
document);